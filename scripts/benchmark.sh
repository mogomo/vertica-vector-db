#!/usr/bin/env bash
# Benchmark on SIFT1M (1,000,000 vectors of 128 dimensions, 10,000 queries, ground truth), or on
# 1,000,000 generated vectors of 128 dimensions: the engine alone, and the SQL statements in every
# deploy mode.
#
...
#
#   --data_dir   where sift_base.fvecs, sift_query.fvecs and sift_groundtruth.ivecs are. Without it the
#                benchmark generates 1M vectors (scripts/load_dataset.sh --generate; tables gen_*, indexes
#                gen, gen_hnsw, gen_sq8), the engine part runs on random data, and recall is measured
#                against the exact (flat) index instead of a ground truth
...
# sift_sq8 (gen, gen_hnsw, gen_sq8 on generated data; build, load); incremental refresh against the size
...
# loaded right before the refresh. Takes 40 to 50 minutes on 8 cores.
...
DS=sift
[ -n "$DATA_DIR" ] || DS=gen
IX=$DS HX=${DS}_hnsw QX=${DS}_sq8
if [ -n "$DATA_DIR" ]; then LOAD=(--dataset=sift --dir="$DATA_DIR" --schema="$SCHEMA")
else LOAD=(--dataset=gen --generate=128 --rows=1000000 --queries=10000 --schema="$SCHEMA"); fi
part() { [[ ",$PARTS," == *",$1,"* ]]; }
OUT="build/benchmark-$(date +%Y%m%d-%H%M%S).txt"
if [ "$ECHO_ONLY" = yes ]; then
    echo "scripts/load_dataset.sh ${LOAD[*]}"
    echo "CALL vvector.register_index('$IX', '$SCHEMA.${DS}_base', 'id', 'vec', 'del', 'ts', 'l2', 0, 'flat'); CALL vvector.refresh_index('$IX');"
    echo "CALL vvector.register_index('$HX', '$SCHEMA.${DS}_base', 'id', 'vec', 'del', 'ts', 'l2', 0, 'hnsw'); CALL vvector.refresh_index('$HX');"
...
if [ "$SKIP_LOAD" = no ] || [ "$(sql "SELECT COUNT(*) FROM v_catalog.tables WHERE LOWER(table_schema) = LOWER('$SCHEMA') AND table_name = '${DS}_base'")" = 0 ]; then
    echo; echo "== load"
    [ -n "$DATA_DIR" ] || [ -x build/tools/fvecs ] || make -s tools > /dev/null
    scripts/load_dataset.sh "${LOAD[@]}" 2>&1 | grep -v NOTICE
fi

if part engine; then
echo; echo "== engine alone (no Vertica): make bench"
make -s bench ${DATA_DIR:+DATA_DIR="$DATA_DIR"} ${HNSWLIB:+HNSWLIB_DIR="$HNSWLIB"} 2>&1 | grep -vE '^(g\+\+|c\+\+|==)'
fi
...
sql "CALL vvector.register_index('$IX', '$SCHEMA.${DS}_base', 'id', 'vec', 'del', 'ts', 'l2', 0, 'flat');" > /dev/null 2>&1
start=$(date +%s%N)
...
sql "CALL vvector.register_index('$HX', '$SCHEMA.${DS}_base', 'id', 'vec', 'del', 'ts', 'l2', 0, 'hnsw');" > /dev/null 2>&1
start=$(date +%s%N)
...
sql "CALL vvector.register_index('$QX', '$SCHEMA.${DS}_base', 'id', 'vec', 'del', 'ts', 'l2', 0, 'hnsw');
...
sql "CALL vvector.unregister_index('${DS}_inc_f');" > /dev/null 2>&1 || true
sql "CALL vvector.unregister_index('${DS}_inc_h');" > /dev/null 2>&1 || true
sql "DROP TABLE IF EXISTS $SCHEMA.${DS}_inc CASCADE;
CREATE TABLE $SCHEMA.${DS}_inc (id INT NOT NULL, vec ARRAY[FLOAT], del BOOLEAN NOT NULL DEFAULT FALSE,
...
INSERT INTO $SCHEMA.${DS}_inc (id, vec, ts) SELECT id, vec, CLOCK_TIMESTAMP() - INTERVAL '1 day' FROM $SCHEMA.${DS}_base WHERE id < 900000;
COMMIT;" > /dev/null
...
sql "CALL vvector.register_index('${DS}_inc_f', '$SCHEMA.${DS}_inc', 'id', 'vec', 'del', 'ts', 'l2', 0, 'flat');" > /dev/null 2>&1
sql "CALL vvector.register_index('${DS}_inc_h', '$SCHEMA.${DS}_inc', 'id', 'vec', 'del', 'ts', 'l2', 0, 'hnsw');" > /dev/null 2>&1
echo "first build, 900,000 vectors:"
refresh_timed ${DS}_inc_f
refresh_timed ${DS}_inc_h
next=900000 gone=0
for adds in 0 100 1000 10000 50000; do
    dels=$((adds / 2))
    echo "change of $adds adds and $dels deletes:"
    if [ "$adds" -gt 0 ]; then
        sql "INSERT INTO $SCHEMA.${DS}_inc (id, vec) SELECT id, vec FROM $SCHEMA.${DS}_base WHERE id >= $next AND id < $((next + adds));
             INSERT INTO $SCHEMA.${DS}_inc (id, del) SELECT id, TRUE FROM $SCHEMA.${DS}_base WHERE id >= $gone AND id < $((gone + dels)); COMMIT;" > /dev/null
    ...
    if [ "$adds" -eq 0 ]; then
        ...
        for x in ${DS}_inc_f ${DS}_inc_h; do
...
refresh_timed ${DS}_inc_f full
refresh_timed ${DS}_inc_h full
sql "CALL vvector.unregister_index('${DS}_inc_f');" > /dev/null 2>&1 || true
sql "CALL vvector.unregister_index('${DS}_inc_h');" > /dev/null 2>&1 || true
sql "DROP TABLE IF EXISTS $SCHEMA.${DS}_inc CASCADE;" > /dev/null
fi

Q0="SELECT qvec FROM $SCHEMA.${DS}_query WHERE qid = 0"
V="qid, qvec, id, vec, del, ver, snapshot_id"
part search && for m in ${MODES//,/ }; do
    echo; echo "== FENCED=$m"
    scripts/deploy.sh --fenced="$m" > /dev/null 2>&1
    if [ "$m" = "${MODES%%,*}" ]; then
        for i in 1 2 3; do sql "SELECT /*+LABEL(${TAG}_scan)*/ b.id, VECTOR_L2(b.vec, q.qvec) AS d FROM $SCHEMA.${DS}_base b CROSS JOIN ($Q0) q ORDER BY d LIMIT 10" > /dev/null; done
    ...
    QS="SELECT qid, qvec, NULL, NULL, NULL, NULL, NULL FROM $SCHEMA.${DS}_query WHERE qid < $BATCH"
    ...
    batch vknn "vknn HNSW (balanced), $BATCH query rows in one statement" \
        "SELECT q.qid, vvector.vknn(q.qvec USING PARAMETERS index_name='$HX', k=10) FROM $SCHEMA.${DS}_query q WHERE q.qid < $BATCH"
done
...
if [ -n "$DATA_DIR" ]; then
    ...
    # No ground truth for generated data: the exact answers of the flat index take its place.
    echo; echo "== recall@10 against the exact search (flat index $IX), $BATCH queries"
    sql "DELETE FROM $SCHEMA.${DS}_gt; INSERT INTO $SCHEMA.${DS}_gt (qid, rank, id)
         SELECT qid, rank, id FROM (SELECT vvector.vsearch($V USING PARAMETERS index_name='$IX', k=10, precision='exact') OVER()
         FROM (SELECT * FROM $SCHEMA.${IX}_snap UNION ALL SELECT qid, qvec, NULL, NULL, NULL, NULL, NULL FROM $SCHEMA.${DS}_query WHERE qid < $BATCH) x) e; COMMIT;" > /dev/null
fi
recall() {   # WHAT INDEX PARAMS
    printf "%-58s %12s\n" "$1" "$(sql "SELECT (COUNT(*) / ($BATCH * 10.0))::NUMERIC(6,4) FROM
     (SELECT vvector.vsearch($V USING PARAMETERS index_name='$2', k=10$3) OVER()
      FROM (SELECT * FROM $SCHEMA.${2}_snap UNION ALL SELECT qid, qvec, NULL, NULL, NULL, NULL, NULL FROM $SCHEMA.${DS}_query WHERE qid < $BATCH) x) r
     JOIN $SCHEMA.${DS}_gt g ON g.qid = r.qid AND g.id = r.id AND g.rank <= 10")"
}
